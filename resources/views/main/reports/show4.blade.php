    ...
    <table class="table table-hover table-bordered">
        ...
        <tbody>
            @foreach($data as $key => $value)
                <tr>
                    <td>
                        <a href="https://www.google.com/maps/dir/{{$value->originzip}},+USA/"
                           target="_blank" class="table1ancher">
                            <i class="fa fa-map-marker" style="color:green;" aria-hidden="true"></i>
                            <span>  {{$value->origincity . "-".$value->originstate ."-" .$value->originzip  }}</span>
                        </a>
                    ...
                    <td>
                        <a href="https://www.google.com/maps/dir/{{$value->destinationzip }},+USA/"
                           target="_blank" class="table1ancher">
                            <i class="fa fa-map-marker" style="color:red;" aria-hidden="true"></i>
                           <span> {{$value->destinationcity . "-".$value->destinationstate ."-" .$value->destinationzip  }}</span>
                        </a>
                        @if($value->daddress)
                    ...
                    <td>
                        ...
                        @foreach($ymk as $val2)
                            ...
                            @endif
                        @endforeach
                        <b> Order ID# </b> <a target="_blank" href="/searchData?search={{$value->id}}">{{$value->id}}</a>
                        <br>
                        ...
                        @if($value->pstatus >= 10 && $value->pstatus <= 14)
                            @if($value->already_auction_storage == 1)
                                ...
                                @if(isset($value->already_storage_date) && isset($value->already_storage_end_date))
                                    @if(\Carbon\Carbon::parse($value->already_storage_date)->format('Y-m-d') < \Carbon\Carbon::parse($value->already_storage_end_date)->format('Y-m-d'))
                                        <?php 
                            ...
                            @if($value->late_pickup_auction_storage == 1)
                                ...
                                @if(isset($value->late_pickup_storage_date) && isset($value->late_pickup_storage_end_date))
                                    @if(\Carbon\Carbon::parse($value->late_pickup_storage_date)->format('Y-m-d') < \Carbon\Carbon::parse($value->late_pickup_storage_end_date)->format('Y-m-d'))
                                        <?php 
                        ...
                        @if($value->pstatus == 11 || $value->pstatus == 12)
                            @if($value->storage_id > 0)
                                ...
                                <br>
                                <b>Storage Move Date:</b> {{\Carbon\Carbon::parse($value->storage_date)->format('M,d Y')}}<br>
                                @if(!empty($value->storage_move_date))
                                <b>Storage End Date:</b> {{\Carbon\Carbon::parse($value->storage_move_date)->format('M,d Y')}}<br>
                    ...
                    <td>
                        Book Price: @if(!empty($value->payment)) ${{$value->payment}} @else N/A @endif
                        <br>
                    ...
                    <td>
                        Order Created Date: {{\Carbon\Carbon::parse($value->created_at)->format('M,d Y h:i A')}} <br>
                        Order Updated Date: {{\Carbon\Carbon::parse($value->created_at)->format('M,d Y h:i A')}}
                    </td>
                </tr>
            @endforeach
        </tbody>
