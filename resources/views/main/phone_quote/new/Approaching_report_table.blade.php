        ...
        <table class="table table-bordered table-sm col-lg-2 fs-18 text-center pd-2 bd-l" role="grid"
            ...
            @foreach ($data as $key => $val)
                ...
                <tr class="parent1{{ $key }}">
                    <td>{{ $key + $i }}</td>
                    @if (!is_null($val->history) && $val->history->isNotEmpty())
                        <td>{{ $val->history->first()->user->name }}</td>
                    @endif
                    <td>{{ $val->name }}</td>

                    <td>{{ count($val->callCount) }}</td>
                    <td>
                        <br>{{ \Carbon\Carbon::parse($val->callCount[0]->updated_at)->format('M,d Y') }}<br>{{ \Carbon\Carbon::parse($val->callCount[0]->updated_at)->format('h:i A') }}
                    </td>
                    <td>
                        @if (count($val->history) > 0)
                            <?php echo !is_null($val->history) ? $val->history[0]->history : '' ?>
                        @else
                            No History
                        @endif
                    </td>
...
<script>
    ...
    $(document).on("click", ".get-history", function(e) {
        ...
        // console.log('company_id'+company_id, 'CompanyName'+CompanyName, 'user_id'+user_id);
        console.log('Yessss');
        ...
        $.ajax({
            url: '{{ route('history.historyApproaching') }}',
            type: 'GET',
            data: {
