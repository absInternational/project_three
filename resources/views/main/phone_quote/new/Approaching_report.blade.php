    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="card">
                ...
                @endphp
                @if ( Auth::user()->role == 1 || Auth::user()->role == 9 || in_array('70', $phoneaccess))
                    <div class="card-header">
                        <div class="container-fluid">
                            <div class="col-lg-12 p-0">
                                    <?php

                                    $user = \App\User::with('userRole')->join('order', 'user.id', '=', 'order.approaching_user')
                                        ->leftJoin('roles', 'user.role', '=', 'roles.id')
                                        ->select('user.id', 'user.name', 'user.last_name', 'roles.name as role_name','user.slug','user.role')
                                        ->where('user.role',2)
                                        ->where('user.status',1)
                                        ->groupBy('user.id')
                                        ->get();
                ...
                <div class="card-body">
                    <div id="table_data">
                        @include('main.phone_quote.new.Approaching_report_table')
                    </div>
                </div>
            </div>
        </div>
...
@section('extraScript')
    ...
    <script>
        ...
        jQuery(document).ready(function ($) {
            ...
            $('#orderTaker').on('change', function (ev, picker) {
                filterData();
            });
            ...
            function filterData() {
                ...
                var url = "{{ route('filter.approaching_reporting') }}";
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        html += "<table id='example' class='table table-bordered dt-responsive table-striped align-middle text-center' role='grid' aria-describedby=''>";
                        ...
                        $.each(data, function (index, val) {
                            ...
                            html += "<td>" + (index + 1) + "</td>";
                            html += "<td>" + (val['user']['name'] || "No User") + "</td>";
                            html += "<td>" + (val['name'] || "") + "</td>";

                            // Call Count column
                            if (val['auto_order']['call_count'] && val['auto_order']['call_count'].length) {
                                html += "<td>" + val['auto_order']['call_count'].length + "</td>";
                                html += "<td>" + (val['auto_order']['call_count'][0]['updated_at'] || "No updated_at") + "</td>";
                            } else {
                            ...
                            // Action column
                            html += "<td>";
                            html += "<button type='button' class='btn btn-primary get-history' data-toggle='modal' data-target='#exampleModal7'>View History";
                            html += "<input hidden type='text' class='Company-ID' value='" + (val['orderId'] || "") + "'>";
                            html += "<input hidden type='text' class='User-ID' value='" + (val['user'] && val['user']['id'] ? val['user']['id'] : "") + "'>";
                            html += "<input hidden type='text' class='Company-Name' value='" + (val['name'] || "") + "'>";
                            html += "</button>";
                            html += "</td>";
                        ...
                        // Close SweetAlert loader once data is loaded
                        Swal.close();
                    },
                    error: function (error) {
                        console.error("Error:", error);
                        // Close SweetAlert and show an error message
                        Swal.close();
                        Swal.fire({
                            icon: 'error',
                            title: 'Oops...',
