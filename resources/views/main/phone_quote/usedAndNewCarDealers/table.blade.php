            ...
            @foreach ($data as $key => $val)
                ...
                <tr class="parent1{{ $key }}">
                    ...
                    <td>
                        ...
                        @else
                            <form method="POST" action="{{ route('autosApproach.save.email') }}"
                                data-compid="{{ $val->id }}">
                                @csrf
                    ...
                    <td>
                        @if ($val->category != null)
                            <span class="categoryText">{{ $val->category }}</span>
                            <button class="btn btn-sm btn-primary editCategoryBtn"
                                data-compid="{{ $val->id }}">Edit</button>
                        @else
                            <form class="addEditCategoryForm" data-compid="{{ $val->id }}">
                                @csrf
                                <input type="hidden" name="comp_id" value="{{ $val->id }}">
                                ...
                                <button type="button" class="btn btn-sm btn-primary submitCategoryBtn">Add</button>
                            </form>

                            <!-- Alert for invalid category (if needed) -->
                            <div class="alert alert-danger mt-2" style="display: none;"
                                id="invalidCategoryAlert_{{ $val->id }}">
                    ...
                    <script>
                        $(document).ready(function() {
                            ...
                            $('.editCategoryBtn').on('click', function() {
                                ...
                                var categoryText = $(this).siblings('.categoryText').text();
                                var editForm = '<form class="editCategoryForm" data-compid="' + compId + '">' +
                                    '@csrf' +
                                    ...
                                    '<option value="Used Car Dealer">Used Car Dealer</option>' +
                                    '</select>' +
                                    '<button type="button" class="btn btn-sm btn-primary submitCategoryBtn">Save</button>' +
                                    '</form>';
                                $(this).siblings('.categoryText').replaceWith(editForm);
                                $('select[name="category"]').val(categoryText);
                            });

                            // Handle form submission with Ajax
                            $(document).on('click', '.submitCategoryBtn', function() {
                                var form = $(this).closest('form');
                                var formData = form.serialize();

                                // Validate category here if needed
                                var categoryInput = form.find('select[name="category"]');
                                var categoryValue = categoryInput.val();
                                if (categoryValue === '') {
                                    $('#invalidCategoryAlert_' + form.data('compid')).show();
                                    return;
                                } else {
                                    $('#invalidCategoryAlert_' + form.data('compid')).hide();
                                }

                                // Ajax request
                                $.ajax({
                                    url: '{{ route('autosApproach.save.category') }}',
                                    type: 'POST',
                                    data: formData,
                                    success: function(response) {
                                        // Replace select with updated text
                                        var updatedCategoryText = '<span class="categoryText">' + response
                                            .category + '</span>';
                                        form.replaceWith(updatedCategoryText);
                                    },
                                    error: function(error) {
                                        console.error('Error:', error);
                                        // Handle errors if needed
