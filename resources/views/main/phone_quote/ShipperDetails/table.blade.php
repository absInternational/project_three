...
<div class="table-responsive" id="usedAndNewTableBody">
    ...
    <table class="table table-bordered table-sm col-lg-2 fs-18 text-center pd-2 bd-l" role="grid" aria-describedby="">
        ...
        <tr>
            ...
            @if (Auth::user()->role === 1 || Auth::user()->role === 3 || Auth::user()->role === 9)
        ...
        @foreach ($data as $key => $val)
            ...
            <tr class="parent1{{ $key }}">
                ...
                <td>
                    ...
                    <script>
                        function openWhatsApp(phone, approachId) {
                            ...
                            $.ajax({
                                url: '{{ route('autosapproachnew.phone.count') }}',
                                type: 'GET',
                                data: {
                ...
                <td>
                    ...
                    @else
                        <form method="POST" action="{{ route('autosApproachNew.save.email') }}"
                              data-compid="{{ $val->id }}">
                            @csrf
                ...
                @if (Auth::user()->role === 3 || Auth::user()->role === 1 || Auth::user()->role === 9)
                    <td>
                ...
                <td>
                    {{ ($val->type == 1 ? 'Shipper' : ($val->type == 2 ? 'Carrier' : 'Broker')) }}
                </td>
                            ...
                            <script>
                                $(document).ready(function() {
                                    ...
                                    $('.addEmailForm[data-compid="{{ $val->id }}"] .submitBtn').on('click', function() {
                                        ...
                                        // Ajax request
                                        $.ajax({
                                            url: '{{ route('autosApproachNew.save.email') }}',
                                            type: 'POST',
                                            data: formData,
                ...
                <div class="modal-body">
                    <div class="col-md-12">
                        <div class="form-group">
                            <form id="addHistoryForm" action="{{ route('autosapproachnew.store.call.history') }}" method="POST"
...
<script>
    ...
    $(".add-history").click(function() {
        ...
        $.ajax({
            url: '{{ route('autosapproachnew.call.history') }}',
            type: 'GET',
            data: {
