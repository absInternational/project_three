...
if (isset($_POST['checkpassword'])){
    $check= \DB::table('report_password')->where('password','=',$_POST['pass'])->first();
    if(!empty($check)){
        $display='yes';
    }else{
        $display='no';
...
@section('content')
    ...
    <div class="page-header">
        ...
        <!--        </li>-->
        <!--        <li class="breadcrumb-item active" aria-current="page"><a href="#">Cancel Bonus</a></li>-->
        <!--    </ol>-->
        <!--</div>-->
        <div class="text-secondary text-center text-uppercase w-100">
            <h1 class="my-4"><b>Cancel Bonus</b></h1>
        </div>
    ...
    <form name="passform" action="/cancel_bonus_2" method="post">
        @csrf
        Enter Password:
        <input type="password" name="pass" />
        <input type="submit" name="checkpassword" value="Go" />
    </form>
    ...
    <div class="container table_body row" style=" position: relative; place-items: center; place-content: center; ">
        ...
        if($display=='yes'){
        ?>
        <form action="/save_cancel_bonus" method="post">
            @csrf
