 ...
 <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" integrity="sha384-..." crossorigin="anonymous">
    ...
    <div class="page-header">
        <div class="text-secondary text-center text-uppercase w-100">
            ...
            <h1 class="my-4"><b>Price Range</b></h1>
                ...
                <div class="card-body">
                    <div class="">
                        <div class="d-flex justify-content-between">                        
                            <h3 class="my-auto">Price Range</h3>
                            @if(Auth::user()->userRole->name == 'Admin')
                            <a class="btn btn-primary" href="{{ route('create.priceRange') }}"><i class="fa fa-plus"></i> Add Price Range</a>
                            @endif
                        </div>
                        <br>
                        <div id="table_data">
                            <div class="table-responsive">
                                <table class="table table-bordered table-sm" style="width:100%" id="" role="grid">
                                    <thead>
                                    <tr>
                                        <th class="border-bottom-0">Min Price</th>
                                        <th class="border-bottom-0">Max Price</th>
                                        <th class="border-bottom-0">AddOn Price</th>
                                    ...
                                    @foreach($data as $key => $val)
                                        <tr>
                                            <td>{{ $val->min_price }}</td>
                                            <td>{{ $val->max_price }}</td>
                                            <td>{{ $val->addon_price }}</td>
                                            <td>{{ \Carbon\Carbon::parse($val->created_at)->format('M,d Y h:i A') }}</td>
                                            @if(Auth::user()->userRole->name == 'Admin')
                                            <td>
                                                <div class="btn-group">
                                                        <a class="btn btn-info" href="{{ route('edit.priceRange', $val->id) }}" title="Edit"><i class="fa fa-edit"></i></a>
                                                        <a class="btn btn-danger" href="{{ route('destroy.priceRange', $val->id) }}" title="Delete"><i class="fa fa-trash"></i></a>
                                                </div>
                                            </td>
                                            @endif
                                        </tr>
...
@section('extraScript')
    ...
    //=================onchange-values=============================
        $(".btn-danger").click(function(e){
          if (confirm("Are you Sure? You want to delete this price range permanently!")) {
          } else {
              e.preventDefault();
          }
        })
